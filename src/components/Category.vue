      ...
      >
        <template v-slot:top>
          <v-toolbar flat color="white">
            ...
            <v-dialog v-model="dialog_category" max-width="500px">
              <v-card>
                <v-card-title>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="blue darken-1" text @click="close_dialog_new_category()">Cancelar</v-btn>
                  <v-btn color="blue darken-1" text @click="save_dialog_new_category()">Guardar</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
            <v-dialog v-model="dialog_delete" max-width="290">
              <v-card>
                ...
                <v-card-text>Está seguro que desea eliminar esta categoría. Solo será posible eliminar la categoría si ningún producto está usando esta categoría.</v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    agree_dialog_delete() {
      this.delete_category(this.item_delete.id);
      this.item_delete = [];
      this.dialog_delete = false;
    },
    delete_category(id) {
    ...
    save_dialog_new_category() {
      ...
      if (me.editedIndex == -1) {
        axios
          ...
          .then(function(response) {
            me.clear_new_category();
            me.categories = [];
            me.dialog_category = false;
            me.list();
          })
          .catch(function(error) {
            console.log(error);
    ...
    close_dialog_new_category() {
      this.clear_new_category();
